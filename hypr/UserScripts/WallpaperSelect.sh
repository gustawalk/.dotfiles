...
# Flags
sddm_refresh_flag="$HOME/.config/hypr/flags/.sddm_refresh_flag"
theme_refresh_flag="$HOME/.config/hypr/flags/.theme_refresh_flag"
monitor_change_flag="$HOME/.config/hypr/flags/.second_monitor_change_theme_flag"
same_wallpaper_flag="$HOME/.config/hypr/flags/.same_wallpapers_flag"
...
# Sorting Wallpapers
menu() {
  refresh_state=$(cat "$theme_refresh_flag")
  sddm_state=$(cat "$sddm_refresh_flag")
  second_monitor_change=$(cat "$second_monitor_change_theme_flag")
...
apply_image_wallpaper() {
  ...
  if [[ $(cat $same_wallpaper_flag) -eq 1 ]]; then
  ...
  # Update the "current wallpaper" file for Rofi and effects
  if [[ "$focused_monitor" == "$MAIN_MONITOR" ]]; then
    ln -sf "$image_path" "$HOME/.config/rofi/.current_wallpaper"
    cp -r "$image_path" "$HOME/.config/hypr/wallpaper_effects/.wallpaper_current"
  fi

  if [[ $(cat $theme_refresh_flag) -eq 1 ]]; then
    if [[ ("$focused_monitor" != "$MAIN_MONITOR" && $(cat $monitor_change_flag) -eq 1) || "$focused_monitor" == "$MAIN_MONITOR" ]]; then
  ...
  if [[ $(cat $sddm_refresh_flag) -eq 1 ]]; then
    set_sddm_wallpaper
  fi
}
